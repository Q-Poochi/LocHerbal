    ...
    const load = useCallback(async () => {
        try {
            ...
            const [revRes, chartRes, invRes] = await Promise.all([
                apiClient.get<RevenueSummary>('/accounting/revenue'),
                apiClient.get<RevenuePoint[]>('/admin/dashboard/revenue-by-day', { params: { days: 7 } }),
    ...
    const kpis = [
        {
            ...
            sub: summary ? `${summary.invoiceCount} hóa đơn` : '',
            icon: 'trending_up',
            color: 'text-green-600',
            bg: 'bg-green-50',
        },
        { label: 'Hóa đơn', value: summary ? String(summary.invoiceCount) : '—', sub: '', icon: 'receipt_long', color: 'text-primary', bg: 'bg-primary/10' },
        { label: 'Giá trị TB đơn', value: summary && summary.invoiceCount ? `${Math.round(summary.revenue / summary.invoiceCount / 1000)}K` : '—', sub: '', icon: 'stats', color: 'text-blue-600', bg: 'bg-blue-50' },
        { label: 'Phạm vi', value: summary ? new Date(summary.from).toLocaleDateString('vi-VN', { month: '2-digit', year: 'numeric' }) : '—', sub: 'từ đầu năm', icon: 'payments', color: 'text-orange-600', bg: 'bg-orange-50' },
    ];
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-1 bg-surface-alt rounded-xl p-0.5 border border-border">
                    {['week', 'month', 'quarter', 'year'].map((p) => (
                        <button key={p}
                            onClick={() => { }}
                            className={`px-4 py-1.5 rounded-lg text-[12px] font-bold transition-all ${period === p ? 'bg-white text-primary-700 shadow-sm' : 'text-text-tertiary hover:text-primary-700'}`}>
