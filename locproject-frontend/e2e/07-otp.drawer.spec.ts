...
test.describe('OTP Login (AuthDrawer)', () => {
  test('yêu cầu OTP login hiển thị banner DEV + ô nhập mã', async ({ page }) => {
    ...
    // DEV MODE banner (NEXT_PUBLIC_APP_ENV=development) hiện mã
    await expect(drawer.getByText(/DEV MODE: Mã OTP là \d{6}/)).toBeVisible({ timeout: 10000 })

    // Ô nhập 6 số hiện
    const codeInput = drawer.locator('input[maxlength="6"]')
    await expect(codeInput).toBeVisible()
    // Đếm ngược hiện
    await expect(drawer.getByText(/gửi lại mã/i)).toContainText('(')
  })

  test('xác thực đúng OTP đăng ký → tạo tài khoản + chuyển về trang chủ', async ({ page }) => {
