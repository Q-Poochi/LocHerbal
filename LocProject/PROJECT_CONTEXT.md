...
- **Windows worker pool teardown warning (Info, không phải bug)**:
    Windows worker pool đôi khi log `worker process has failed to exit gracefully` khi chạy multi-worker — đã xác nhận KHÔNG phải leak thật (`--detectOpenHandles --maxWorkers=1` sạch 100%, 270/270 test pass). Nếu warning này xuất hiện lại, không cần hoảng — chỉ điều tra nếu kèm theo test bị treo hoặc CI timeout thật sự.
  - **Sweeper/Reconciliation Cron Job (✅ đã triển khai 16/09/2026) cho Orphaned Orders (Rủi ro Concurrency & DB Failure)**:
    Khi sự cố DB xảy ra trong pha bồi hoàn (compensating logic) của `OrderCreatedListener` (hết hàng hoặc allocate thất bại nhưng không cập nhật được DB sang `allocationStatus: FAILED` hay `status: CANCELLED`), đơn hàng sẽ bị kẹt vĩnh viễn ở trạng thái `status: PENDING` và `allocationStatus: PENDING`. Hệ thống hiện chưa có Cron Job (NestJS Schedule) định kỳ quét các đơn hàng PENDING quá hạn (ví dụ > 15-30 phút) để đối soát, hủy đơn hoặc cảnh báo cho Admin/Ops. Đã triển khai `OrphanedOrderSweeperJob` (cron `*/5 * * * *`, ngưỡng 15 phút): đơn chưa thanh toán → tự hủy + trả tồn kho; đơn đã PAID → KHÔNG tự hủy, gắn cờ [CẦN ADMIN XỬ LÝ] và hiện ở tab ⚠️ Cần xử lý của admin (/admin/orders/flagged). Còn TODO: cảnh báo ra ngoài (email/Slack) cho admin khi có đơn bị gắn cờ.
...
- **Load test race condition oversell → ✅ Đã verify (13/08/2026)**: 3 kịch bản k6 (`LocProject/k6/`) + 1 e2e concurrency (`LocProject/test/concurrency-allocate.e2e-spec.ts`) — xác nhận KHÔNG oversell và KHÔNG allocate một phần:
  - `race-condition-stock.js`: 10 user đồng loạt mua SKU còn 2 (qty=1) → đúng 2 order ALLOCATED, 8 fail 400 InsufficientStock, `qty_reserved=2`. Chạy 3 lần liên tiếp, cả 3 PASS.
  - `no-contention-checkout.js`: 5 user mua SKU A (stock 5) + 5 user mua SKU B (stock 5) cùng lúc → 10/10 thành công, p95 không tăng vọt so với baseline → lock phạm vi per-row, không khóa cả bảng.
  - `multi-item-outofstock-checkout.js`: giỏ 2 dòng (C qty=2 + D qty=2), C hết hàng giữa chừng (user khác mua hết) → toàn bộ checkout 400, không allocate một phần, stock C & D rollback hoàn toàn (verify StockMovement).
  - `test/concurrency-allocate.e2e-spec.ts`: 2 allocate() đồng thời × 5 vòng trên DB thật → đúng 1 fulfilled + 1 `InsufficientStockException`, `qty_reserved=2` cuối.
